...
def main():
    ...
    try:
        devices = [
            {"com_port": "COM6", "baud_rate": 460800, "name": "CC26x2 Master"},
            # {"com_port": "COM6", "baud_rate": 460800, "name": "CC26x2 Passive"},
            {"com_port": "COM3", "baud_rate": 460800, "name": "CC26x2 Passive"},
...
def calculate(LEVELangel,AOAangel1,AOAangel2):
    ...
    if (bool1 | bool2):
        ...
        focal_length = 4.69  # 焦距/毫米
        pixel_size = 1.55  # 单个像素大小/微米
        f = focal_length * 1000 / pixel_size  # 像素焦距

        camera_positon_u = f * (np.sin(azimuth1) / np.cos(azimuth1))  # 相机坐标以摄像头中心建立的坐标系
...
def main1():
    ...
    clf_name = 'kNN'
    clf = KNN()  # 初始化检测器clf
    clf.fit(np.array(y).reshape(-1, 1))  # 使用X_train训练检测器clf
