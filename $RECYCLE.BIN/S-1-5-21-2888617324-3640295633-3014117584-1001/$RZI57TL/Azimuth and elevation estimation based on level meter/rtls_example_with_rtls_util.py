...
def main():
    ...
    try:
        devices = [
            {"com_port": "COM5", "baud_rate": 460800, "name": "CC26x2 Master"},
    ...
    return "It is over for now"
    # return main1()

# Function to calculate azimuth and elevation
def calculate(LEVELangel,AOAangel1,AOAangel2):
    #temp1 = np.pi/36
    #temp2 = np.pi/30
    #print("The correct value is: " + str(temp1))
    #Calculate the Azimuth and Elevation Angle in Radian
    ...
    if (bool1 | bool2):
        ...
        camera_positon_u = f * (np.sin(azimuth1) / np.cos(azimuth1))  # Camera Co-ordinates: The co-ordinate system established by the center of camera
        camera_position_v = f * (np.cos(elvation1) / (np.cos(azimuth1) * np.sin(elvation1)))


        # Co-ordinate conversion: Camera co-ordinate -> GUI co-ordinate
...
def main1():
    ...
    clf_name = 'kNN'
    clf = KNN()  #intialize detector clf 
    clf.fit(np.array(y).reshape(-1, 1))  # use x_train to train detector clf
    # Returns abnormal scores on the training data X_train
    y_train_pred = clf.labels_  # Returns classification label on the training data(0: normal, 1: outlier)
    # y_train_scores = clf.decision_scores_  # Returns the abnormal value on the training data(the larger the score, the more abnormal) result = []
